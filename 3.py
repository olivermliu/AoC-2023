...
def get_start_pos_and_full_part_number(row, col):
    beginning_pos = col
    end_pos = col + 1
    # find beginning of number
    try:
        while data[row][beginning_pos - 1] in string.digits:
            beginning_pos -= 1
    except IndexError:
        pass
    # find end of number
    try:
        while data[row][end_pos] in string.digits:
            end_pos += 1
    except IndexError:
        pass

    return (row, beginning_pos), data[row][beginning_pos:end_pos]


def get_neighboring_part_numbers(row, col):
    neighbors = [
        (row - 1, col - 1),
        (row - 1, col    ),
        (row - 1, col + 1),
    ...
    for (row_no, col_no) in neighbors:
        try:
            int(data[row_no][col_no])   # except (and skip) if out of range or not a number
            pos_tuple, part_number = get_start_pos_and_full_part_number(row_no, col_no)
            neighbors_with_numbers[pos_tuple] = int(part_number)
...
to_be_summed_1 = {} # k: (row,col);  v: part_number
part_2_sum = 0

for symbol in string.punctuation:
    if symbol == ".":
        continue
    for row_number, row_content in enumerate(data):
        start_pos = 0
        while row_content.find(symbol, start_pos) != -1:
            curr_pos = row_content.find(symbol, start_pos)
            neighbor_info = get_neighboring_part_numbers(row_number, curr_pos)
            for pos_data, part_number in neighbor_info:
                to_be_summed_1[pos_data] = part_number
            if symbol == "*" and len(neighbor_info) == 2:
                part_2_sum += prod(neighbor_info.values())
            start_pos = curr_pos + 1
    
print(f"Sum of all part numbers for 1: {sum(to_be_summed_1.values())}")
print(f"Sum of all gear ratios for 2: {part_2_sum}")
